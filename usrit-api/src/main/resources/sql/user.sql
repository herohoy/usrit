...
create table if not exists user(
  ...
  status tinyint(2) not null default 1 comment '用户状态,1:已激活;2:权属用户已完善;3:已冻结;4:黑名单用户;99:已逻辑删除',
...
create table if not exists integral_journal(
  ...
  remark char(255) not null default '' comment '备注',
  primary key (`id`)
)engine=InnoDB auto_increment=1000000000 default charset=utf8 comment='用户积分流水表';
